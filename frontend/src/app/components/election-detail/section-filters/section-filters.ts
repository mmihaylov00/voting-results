...
export class SectionFiltersComponent implements OnInit {
  ...
  @Input() showMissingPositionQuickFilters: boolean = false;
  @Input() positions: { id: string; name: string }[] = [];
  ...
  quickMissingPositionId = signal<string>('');
  quickMissingAnyPosition = signal<boolean>(false);
  ...
  ngOnInit() {
    if (this.initialFilters) {
      ...
      this.quickMissingPositionId.set(this.initialFilters.quickMissingPositionId || '');
      this.quickMissingAnyPosition.set(!!this.initialFilters.quickMissingAnyPosition);
    }
  }
  ...
  constructor() {
    effect(() => {
      this.filterChange.emit({
        ...
        quickMissingPositionId: this.quickMissingPositionId(),
        quickMissingAnyPosition: this.quickMissingAnyPosition(),
      });
    });
  }
  ...
  selectMissingFilter(mode: 'none' | 'any' | 'position', positionId?: string): void {
    if (mode === 'none') {
      this.quickMissingAnyPosition.set(false);
      this.quickMissingPositionId.set('');
      return;
    }
    if (mode === 'any') {
      this.quickMissingAnyPosition.set(true);
      this.quickMissingPositionId.set('');
      return;
    }
    this.quickMissingAnyPosition.set(false);
    this.quickMissingPositionId.set(positionId || '');
  }

  isMissingFilterSelected(mode: 'none' | 'any' | 'position', positionId?: string): boolean {
    if (mode === 'none') return !this.quickMissingAnyPosition() && !this.quickMissingPositionId();
    if (mode === 'any') return this.quickMissingAnyPosition() && !this.quickMissingPositionId();
    return !this.quickMissingAnyPosition() && this.quickMissingPositionId() === (positionId || '');
  }
}
