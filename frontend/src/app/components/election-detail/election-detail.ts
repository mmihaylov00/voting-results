...
import { AutocompleteComponent, AutocompleteOption } from '../ui/autocomplete/autocomplete';
...
type AssignmentDto = {
  ...
  section?: { id: string; sectionId: string; sectionName: string; cityName?: string };
  position?: { id: string; name: string; color?: string };
};

type BannerType = 'sections' | 'people' | null;
...
export class ElectionDetailComponent implements OnInit {
  ...
  compactMappingFallback = signal<boolean>(false);
  ...
  canManageVolunteers(): boolean {
    return this.authService.hasRole(APP_ROLE.ADMIN, APP_ROLE.CAMPAIGN_MANAGER);
  }

  electionId: string | null = null;
  managedSections: ManageSectionDto[] = [];
  managedPeople = signal<ManagePersonDto[]>([]);
  managedPositions = signal<PositionDto[]>([]);
  managedAssignments = signal<AssignmentDto[]>([]);
  bannerType = signal<BannerType>(null);
  uploadModalOpen = signal(false);
  uploadMode = signal<'sections' | 'people'>('sections');
  uploadFile = signal<File | null>(null);
  uploadError = signal<string | null>(null);

  addPersonModalOpen = signal(false);
  addPersonError = signal<string | null>(null);
  addPersonForm = { fullName: '', email: '', phone: '' };
  editPersonModalOpen = signal(false);
  editPersonError = signal<string | null>(null);
  editPersonForm = { id: '', fullName: '', email: '', phone: '' };

  assignModalOpen = signal(false);
  assignError = signal<string | null>(null);
  assignForm = { personId: '', electionSectionId: '', positionId: '' };
  assignPersonInput = '';
  assignSectionInput = '';
  showAssignmentFilterDropdown = signal(false);
  volunteerOnlyUnassigned = false;
  volunteerPositionFilterId = '';
  quickFilterPositionId = '';
  quickFilterAnyMissingPosition = false;

  volunteerSearchTerm = '';
  selectedSectionVolunteerEntries: AssignmentDto[] = [];
  private assignmentByPersonId = new Map<string, AssignmentDto>();
  private assignmentsBySectionCode = new Map<string, AssignmentDto[]>();
  private sectionByCode = new Map<string, ManageSectionDto>();
  private sectionPositionCounts = new Map<string, Record<string, number>>();
  private managedDataLoaded = false;
  ...
  constructor(
    ...
    this.electionService.compactMappingFallback$.subscribe((isFallback) => {
      this.compactMappingFallback.set(isFallback);
    });
  ...
  ngOnInit(): void {
    ...
    this.dateName = this.date;
    this.electionService.dates$.subscribe((dates) => {
      const selected = dates.find(d => d.date === this.date);
      const resolvedDateName = selected?.name ?? this.date;
      queueMicrotask(() => {
        this.dateName = resolvedDateName;
      });
      const prevElectionId = this.electionId;
      this.electionId = selected?.id ?? null;
      if (prevElectionId !== this.electionId) {
        this.managedDataLoaded = false;
      }
      if (this.canManageVolunteers()) {
        this.loadManagedElectionData();
      }
    });
  ...
  applyFilter(): void {
    ...
    const baseResult = filterSections(this.sections, filters, regionAvgTurnoutById);
    const result = this.applySectionQuickFilters(baseResult);
  ...
  private loadManagedElectionData(force = false): void {
    if (!this.electionId || !this.canManageVolunteers()) return;
    if (this.managedDataLoaded && !force) return;

    forkJoin({
      sections: this.manageSectionsService.list(this.electionId).pipe(catchError(() => of([] as ManageSectionDto[]))),
      people: this.managePeopleService.list(this.electionId).pipe(catchError(() => of([] as ManagePersonDto[]))),
      positions: this.positionsService.list().pipe(catchError(() => of([] as PositionDto[]))),
      assignments: this.assignmentsService.list(this.electionId).pipe(catchError(() => of([] as AssignmentDto[]))),
    }).subscribe(({ sections, people, positions, assignments }) => {
      this.managedSections = sections;
      this.managedPeople.set(people);
      this.managedPositions.set(positions);
      this.managedAssignments.set((assignments as AssignmentDto[]).map((assignment) => this.normalizeAssignment(assignment)));
      if (!people.length) {
        this.quickFilterPositionId = '';
        this.quickFilterAnyMissingPosition = false;
      }
      this.rebuildManagedIndexes();
      this.managedDataLoaded = true;
      if (this.isModalOpen()) {
        this.refreshSelectedSectionVolunteerEntries();
      }
  ...
  private rebuildManagedIndexes(): void {
    this.assignmentByPersonId.clear();
    this.assignmentsBySectionCode.clear();
    this.sectionByCode.clear();
    this.sectionPositionCounts.clear();

    for (const section of this.managedSections) {
      this.sectionByCode.set(section.sectionId, section);
      const counts: Record<string, number> = {};
      for (const p of this.managedPositions()) {
        counts[p.id] = 0;
      }
      this.sectionPositionCounts.set(section.sectionId, counts);
    }

    for (const assignment of this.managedAssignments()) {
      this.assignmentByPersonId.set(assignment.personId, assignment);
      const sectionCode = assignment.section?.sectionId || this.managedSections.find((s) => s.id === assignment.electionSectionId)?.sectionId;
      if (!sectionCode) continue;

      const existing = this.assignmentsBySectionCode.get(sectionCode) || [];
      existing.push(assignment);
      this.assignmentsBySectionCode.set(sectionCode, existing);

      const counts = this.sectionPositionCounts.get(sectionCode) || {};
      counts[assignment.positionId] = (counts[assignment.positionId] || 0) + 1;
      this.sectionPositionCounts.set(sectionCode, counts);
    }
  }

  private updateBannerState(): void {
    if (!this.isAdmin() || !this.isFutureElection()) {
      this.bannerType.set(null);
      return;
    }

    if (this.managedSections.length === 0) {
      this.bannerType.set('sections');
      return;
    }

    if (this.managedPeople().length === 0) {
      this.bannerType.set('people');
      return;
    }
  ...
  private isFutureElection(): boolean {
    const parts = this.date.split('.').map((n) => Number(n));
  ...
  openPeopleImportModal(): void {
    this.uploadMode.set('people');
    this.uploadFile.set(null);
    this.uploadError.set(null);
    this.uploadModalOpen.set(true);
  }
  ...
  onUploadFileSelected(event: Event): void {
    ...
    this.uploadFile.set(input.files && input.files.length > 0 ? input.files[0] : null);
  }

  runUploadImport(): void {
    if (!this.electionId || !this.uploadFile()) return;
    this.uploadError.set(null);
    const request$ = this.uploadMode() === 'sections'
      ? this.uploadsService.uploadSections(this.electionId, this.uploadFile() as File)
      : this.uploadsService.uploadPeople(this.electionId, this.uploadFile() as File);

    request$.subscribe({
      next: () => {
        this.closeImportModal();
        this.loadManagedElectionData(true);
      },
      error: (err) => {
        this.uploadError.set(err?.error?.message || 'Неуспешен импорт.');
      },
    });
  }
  ...
  openAddPersonModal(): void {
    ...
    this.addPersonForm = { fullName: '', email: '', phone: '' };
  ...
  createPerson(): void {
    ...
    const fullName = this.addPersonForm.fullName.trim();
    const email = this.addPersonForm.email.trim();
    const phone = this.addPersonForm.phone.trim();
    if (!this.canSubmitAddPerson()) {
      this.addPersonError.set('Попълнете всички полета.');
      return;
    }

    this.managePeopleService.create(this.electionId, {
      fullName,
      email,
      phone,
    }).subscribe({
      next: (person) => {
        ...
        this.managedPeople.set([...this.managedPeople(), person as ManagePersonDto]);
        this.rebuildManagedIndexes();
        this.updateBannerState();
        this.applyFilter();
      },
      error: (err) => this.addPersonError.set(err?.error?.message || 'Неуспешно добавяне.'),
    });
  }

  openEditPersonModal(person: ManagePersonDto): void {
    this.editPersonError.set(null);
    this.editPersonForm = {
      id: person.id,
    ...
    this.editPersonModalOpen.set(true);
  }

  closeEditPersonModal(): void {
    this.editPersonModalOpen.set(false);
  }

  savePersonDetails(): void {
    if (!this.electionId || !this.editPersonForm.id) return;
    if (!this.canSubmitEditPerson()) {
      this.editPersonError.set('Попълнете всички полета.');
      return;
    }

    const payload = {
      fullName: this.editPersonForm.fullName.trim(),
      email: this.editPersonForm.email.trim(),
      phone: this.editPersonForm.phone.trim(),
    };

    this.managePeopleService.update(this.electionId, this.editPersonForm.id, payload).subscribe({
      next: (person) => {
        this.applyUpdatedPerson(person as ManagePersonDto);
        this.closeEditPersonModal();
      },
      error: (err) => this.editPersonError.set(err?.error?.message || 'Неуспешно обновяване.'),
    });
  }

  canSubmitAddPerson(): boolean {
    return !!this.addPersonForm.fullName.trim() && !!this.addPersonForm.email.trim() && !!this.addPersonForm.phone.trim();
  }

  canSubmitEditPerson(): boolean {
    return !!this.editPersonForm.fullName.trim() && !!this.editPersonForm.email.trim() && !!this.editPersonForm.phone.trim();
  }

  deletePerson(person: ManagePersonDto): void {
    if (!this.electionId) return;
    if (!confirm(`Сигурни ли сте, че искате да изтриете "${person.fullName}"?`)) {
      return;
    }

    const previousPeople = [...this.managedPeople()];
    const previousAssignments = [...this.managedAssignments()];
    this.removePersonLocal(person.id);
    this.managePeopleService.remove(this.electionId, person.id).subscribe({
      next: () => undefined,
      error: () => {
        this.managedPeople.set(previousPeople);
        this.managedAssignments.set(previousAssignments);
        this.rebuildManagedIndexes();
        if (this.isModalOpen()) {
          this.refreshSelectedSectionVolunteerEntries();
        }
  ...
  get volunteersData(): ManagePersonDto[] {
    const term = this.volunteerSearchTerm.trim().toLowerCase();
    let list = this.managedPeople();
    if (this.volunteerOnlyUnassigned) {
      list = list.filter((p) => !this.assignmentByPersonId.has(p.id));
    }
    if (this.volunteerPositionFilterId) {
      list = list.filter((p) => {
        const a = this.assignmentByPersonId.get(p.id);
        return !!a && a.positionId === this.volunteerPositionFilterId;
      });
    }
    if (!term) return list;
  ...
  getAssignmentForPerson(personId: string): AssignmentDto | undefined {
    return this.assignmentByPersonId.get(personId);
  }

  unassignPerson(personId: string): void {
    const assignment = this.assignmentByPersonId.get(personId);
    if (!assignment || !this.electionId) return;
    const previous = [...this.managedAssignments()];
    this.removeAssignmentLocal(assignment.id, personId);
    this.assignmentsService.remove(this.electionId, assignment.id).subscribe({
      next: () => undefined,
      error: () => {
        this.managedAssignments.set(previous);
        this.rebuildManagedIndexes();
        if (this.isModalOpen()) {
          this.refreshSelectedSectionVolunteerEntries();
        }
  ...
  openAssignModal(personId?: string, sectionId?: string): void {
    this.assignError.set(null);
    this.assignForm = {
      personId: personId || '',
      electionSectionId: sectionId || '',
      positionId: '',
    };
    this.assignPersonInput = personId ? this.personOptionLabel(this.managedPeople().find((p) => p.id === personId)) : '';
    this.assignSectionInput = sectionId ? this.sectionOptionLabel(this.managedSections.find((s) => s.id === sectionId)) : '';
  ...
  submitAssignment(): void {
    ...
    if (!this.canSubmitAssignment()) {
      this.assignError.set('Изберете доброволец, секция и позиция.');
      return;
    }

    this.assignmentsService.create({
      electionId: this.electionId,
      personId: this.assignForm.personId,
      electionSectionId: this.assignForm.electionSectionId,
      positionId: this.assignForm.positionId,
    }).subscribe({
      next: (created) => {
        this.closeAssignModal();
        this.addAssignmentLocal(created as AssignmentDto);
      },
      error: (err) => this.assignError.set(err?.error?.message || 'Неуспешно разпределение.'),
    });
  }

  get assignablePeople(): ManagePersonDto[] {
    const selectedPersonId = this.assignForm.personId;
    return this.managedPeople().filter((p) => {
      const assigned = this.assignmentByPersonId.has(p.id) && p.id !== selectedPersonId;
      if (assigned) return false;
      return true;
    });
  }

  get assignableSections(): ManageSectionDto[] {
    if (!this.regionId || this.regionId === 'all') {
      return this.managedSections;
    }
    const normalizedRegion = this.regionId.padStart(2, '0');
    return this.managedSections.filter((section) => section.sectionId.startsWith(normalizedRegion));
  }

  get assignablePositions(): PositionDto[] {
    return this.managedPositions();
  }

  get personAutocompleteOptions(): AutocompleteOption[] {
    return this.assignablePeople.map((p) => ({
      id: p.id,
      label: this.personOptionLabel(p),
      description: p.phone || undefined,
    }));
  }

  get sectionAutocompleteOptions(): AutocompleteOption[] {
    return this.assignableSections.map((s) => ({
      id: s.id,
  ...
  personOptionLabel(person?: ManagePersonDto): string {
    if (!person) return '';
    return person.email ? `${person.fullName} (${person.email})` : person.fullName;
  }

  sectionOptionLabel(section?: ManageSectionDto): string {
    if (!section) return '';
    return `${section.sectionId} - ${section.cityName}`;
  }

  onAssignPersonInput(value: string): void {
    this.assignForm.personId = value;
  }

  onAssignSectionInput(value: string): void {
    this.assignForm.electionSectionId = value;
  }

  selectAssignmentPosition(positionId: string): void {
    this.assignForm.positionId = positionId;
  }

  canSubmitAssignment(): boolean {
    if (!this.assignForm.personId || !this.assignForm.electionSectionId || !this.assignForm.positionId) {
      return false;
    }
    const validPerson = this.assignablePeople.some((person) => person.id === this.assignForm.personId);
    const validSection = this.assignableSections.some((section) => section.id === this.assignForm.electionSectionId);
    const validPosition = this.assignablePositions.some((position) => position.id === this.assignForm.positionId);
    return validPerson && validSection && validPosition;
  }

  toggleVolunteerFiltersDropdown(): void {
    this.showAssignmentFilterDropdown.update((v) => !v);
  }

  clearVolunteerFilters(): void {
    this.volunteerOnlyUnassigned = false;
    this.volunteerPositionFilterId = '';
    this.showAssignmentFilterDropdown.set(false);
  }

  getPositionBadgeStyle(positionId?: string): Record<string, string> {
    const color = this.managedPositions().find((p) => p.id === positionId)?.color || '#64748b';
    return {
  ...
  getPositionName(positionId?: string): string {
    return this.managedPositions().find((p) => p.id === positionId)?.name || '-';
  }

  unassignByAssignmentId(id: string): void {
    if (!this.electionId) return;
    const previous = [...this.managedAssignments()];
    this.removeAssignmentLocal(id);
    this.assignmentsService.remove(this.electionId, id).subscribe({
      next: () => undefined,
      error: () => {
        this.managedAssignments.set(previous);
        this.rebuildManagedIndexes();
  ...
  private addAssignmentLocal(created: AssignmentDto): void {
    const normalized = this.normalizeAssignment(created);
    if (!normalized.id) {
      this.loadManagedElectionData(true);
      return;
    }
    const current = this.managedAssignments().filter((assignment) => assignment.id !== normalized.id);
    this.managedAssignments.set([normalized, ...current]);
    this.rebuildManagedIndexes();
    if (this.isModalOpen()) {
      this.refreshSelectedSectionVolunteerEntries();
    }
  ...
  private removeAssignmentLocal(assignmentId: string, personId?: string): void {
    const assignmentKey = String(assignmentId);
    this.managedAssignments.set(
      this.managedAssignments().filter((assignment) => {
        const keepById = String(assignment.id) !== assignmentKey;
        if (!personId) {
          return keepById;
        }
        return keepById && assignment.personId !== personId;
      }),
    );
    this.rebuildManagedIndexes();
    if (this.isModalOpen()) {
  ...
  private normalizeAssignment(assignment: AssignmentDto): AssignmentDto {
    const positionId = assignment.positionId || assignment.roleId || '';
    const personModel = this.managedPeople().find((person) => person.id === assignment.personId);
    const sectionModel = this.managedSections.find((section) => section.id === assignment.electionSectionId);
    const positionModel = this.managedPositions().find((position) => position.id === positionId);

    return {
      ...assignment,
      positionId,
      person: assignment.person || (personModel ? { id: personModel.id, fullName: personModel.fullName } : undefined),
      section: assignment.section || (sectionModel
        ? {
          id: sectionModel.id,
          sectionId: sectionModel.sectionId,
          sectionName: sectionModel.sectionName,
          cityName: sectionModel.cityName,
        }
        : undefined),
      position: assignment.position || (positionModel
        ? {
          id: positionModel.id,
  ...
  private applyUpdatedPerson(person: ManagePersonDto): void {
    const people = this.managedPeople().map((item) => (item.id === person.id ? person : item));
    this.managedPeople.set(people);
    this.managedAssignments.set(
      this.managedAssignments().map((assignment) =>
  ...
  private removePersonLocal(personId: string): void {
    this.managedPeople.set(this.managedPeople().filter((person) => person.id !== personId));
    this.managedAssignments.set(this.managedAssignments().filter((assignment) => assignment.personId !== personId));
    this.rebuildManagedIndexes();
    if (this.isModalOpen()) {
      this.refreshSelectedSectionVolunteerEntries();
    }
  ...
  openAssignForSection(sectionCode: string): void {
    const section = this.sectionByCode.get(sectionCode);
    if (!section) return;
    this.openAssignModal(undefined, section.id);
  }

  getSectionPositionCounts(sectionCode: string): Record<string, number> {
    return this.sectionPositionCounts.get(sectionCode) || {};
  }

  hasAnyMissingPosition(sectionCode: string): boolean {
    const counts = this.getSectionPositionCounts(sectionCode);
    const positions = this.managedPositions();
    if (positions.length === 0) return false;
    return positions.some((p) => (counts[p.id] || 0) === 0);
  }

  private applySectionQuickFilters(items: Section[]): Section[] {
    if (!this.canManageVolunteers()) return items;

    return items.filter((s) => {
      const code = s.sectionId;
      if (this.quickFilterPositionId) {
        const counts = this.getSectionPositionCounts(code);
        if ((counts[this.quickFilterPositionId] || 0) > 0) {
          return false;
        }
      }
      if (this.quickFilterAnyMissingPosition && !this.quickFilterPositionId) {
        return this.hasAnyMissingPosition(code);
      }
      return true;
    });
  }
  ...
  onFilterChange(filters: SectionFilters): void {
    ...
    this.quickFilterPositionId = filters.quickMissingPositionId || '';
    this.quickFilterAnyMissingPosition = !!filters.quickMissingAnyPosition;
  ...
  onAssignFromSectionModal(): void {
    ...
    const sectionCode = current.sectionId;
    if (!sectionCode) return;
    const managedSection = this.sectionByCode.get(String(sectionCode));
    this.openAssignModal(undefined, managedSection?.id);
  }

  private refreshSelectedSectionVolunteerEntries(): void {
    const current = this.currentSectionData as any;
    if (!current) {
      this.selectedSectionVolunteerEntries = [];
      return;
    }
    if (Array.isArray(current.sections)) {
      const sectionIds = current.sections.map((s: Section) => String(s.sectionId));
      const rows: AssignmentDto[] = [];
      for (const sectionId of sectionIds) {
        const source = this.assignmentsBySectionCode.get(sectionId) || [];
        source.forEach((a) => rows.push(a));
      }
      this.selectedSectionVolunteerEntries = rows;
      return;
    }
    const sectionCode = String(current.sectionId || '');
    this.selectedSectionVolunteerEntries = this.assignmentsBySectionCode.get(sectionCode) || [];
  }
