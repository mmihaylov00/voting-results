...
type SectionVolunteerAssignment = {
  id: string;
  section?: { sectionId?: string; sectionName?: string; cityName?: string };
...
export class SectionDetailModalComponent implements OnInit, OnChanges {
  ...
  @Input() historicalData: { [date: string]: any } = {};
  @Input() volunteers: SectionVolunteerAssignment[] = [];
  @Input() showVolunteersTab = false;
  @Input() canManageVolunteers = false;
  @Output() close = new EventEmitter<void>();
  @Output() openCandidate = new EventEmitter<RegionCandidate>();
  @Output() assignVolunteer = new EventEmitter<void>();
  @Output() unassignVolunteer = new EventEmitter<string>();
  ...
  activeTab = signal<'parties' | 'candidates' | 'volunteers'>('parties');
  ...
  ngOnInit() {
    ...
    this.allData = this.historicalData || {};
  }
  ...
  onAssignVolunteer(): void {
    this.assignVolunteer.emit();
  }

  onUnassignVolunteer(assignmentId: string): void {
    this.unassignVolunteer.emit(assignmentId);
  }
  ...
  get currentColumns(): TableColumn[] {
    if (this.activeTab() === 'parties') return this.partyColumns;
    if (this.activeTab() === 'candidates') return this.candidateColumns;
    return [];
  }

  get currentVisibleColumns(): Set<string> {
    if (this.activeTab() === 'parties') return this.visiblePartyColumns();
    if (this.activeTab() === 'candidates') return this.visibleCandidateColumns();
    return new Set<string>();
  }

  get currentRowCount(): number {
    if (this.activeTab() === 'parties') return this.filteredPartyResults.length;
    if (this.activeTab() === 'candidates') return this.filteredCandidateResults.length;
    return this.volunteers.length;
  }

  get currentRowCountLabel(): string {
    if (this.activeTab() === 'parties') return 'Партии';
    if (this.activeTab() === 'candidates') return 'Кандидати';
    return 'Доброволци';
  }

  get currentSearchTerm(): string {
    if (this.activeTab() === 'parties') return this.partySearchTerm;
    if (this.activeTab() === 'candidates') return this.candidateSearchTerm;
    return '';
  }

  onSearchTermChange(term: string): void {
    if (this.activeTab() === 'volunteers') return;
