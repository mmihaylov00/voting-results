...
import { ManageSectionsService } from '../../services/manage-sections.service';
...
export class RegionListComponent implements OnInit, AfterViewInit {
  ...
  bannerType = signal<'sections' | 'people' | null>(null);
  ...
  ngOnInit() {
    this.route.params.subscribe(params => {
      ...
      this.electionService.dates$.subscribe((dates) => {
        ...
        this.dateName = selected?.name ?? this.date;
  ...
  private loadBannerState(): void {
    if (!this.isAdmin() || !this.isFutureElection() || !this.electionId) {
      this.bannerType.set(null);
      return;
    }
    this.manageSectionsService.list(this.electionId).subscribe({
      next: (sections) => {
        if (!sections.length) {
          this.bannerType.set('sections');
          return;
        }
        this.managePeopleService.list(this.electionId as string).subscribe({
          next: (people) => this.bannerType.set(people.length ? null : 'people'),
          error: () => this.bannerType.set('people'),
        });
      },
      error: () => this.bannerType.set('sections'),
    });
  }
  ...
  runUploadImport(): void {
    ...
    request$.subscribe({
      ...
      error: (err) => this.uploadError.set(err?.error?.message || 'Неуспешен импорт.'),
    });
  }
