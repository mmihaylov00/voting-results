...
@Component({
  ...
  templateUrl: './people-upload.html',
})
export class PeopleUploadComponent {
  ...
  onPreview() {
    ...
    this.uploads.previewPeople(this.electionId, this.selectedFile() as File).subscribe({
  ...
  onUpload() {
    ...
    this.error.set(null);
    this.validationErrors.set([]);
    this.uploads.uploadPeople(this.electionId, this.selectedFile() as File).subscribe({
      next: (data) => this.preview.set(data),
