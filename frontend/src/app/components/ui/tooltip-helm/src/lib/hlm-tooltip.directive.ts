...
export class HlmTooltipDirective implements OnDestroy {
  ...
  private resolveDateLabel(date: string): string {
    const found = this.electionService.getDates().find((d) => d.date === date);
    return found?.name || date;
  }
    ...
    if (typeof this.comparisons === 'string') {
      ...
      content += `
        <div class="flex justify-between gap-4 text-[10px] whitespace-nowrap items-center">
          <span class="opacity-70">${this.resolveDateLabel(c.d)}:</span>
