...
const baseDataDir = process.env.BASE_DATA_DIR
  ? path.resolve(process.env.BASE_DATA_DIR)
...
const dates = fs
  .readdirSync(baseDataDir, {withFileTypes: true})
  .filter((entry) => entry.isDirectory() && /^\d{4}\.\d{2}\.\d{2}$/.test(entry.name))
  .map((entry) => entry.name)
  .sort()
  .reverse();
const dateNameByDate = Object.fromEntries(dates.map((date) => [date, date]));
